...
const cv::Size img_size = cv::Size(62, 62);                      //图片归一化的尺寸；

const int IsOverLapping = 1;                                     //PDMs提取时是否采用重叠：0表示不重叠，1表示重叠。

const std::string filepath = "D:\\data80\\";                     //文件路径

const int trainnum = 10;                                          //每类样本的训练数；

const int samplenum = 200;                                       //每类样本的样本数；

const int classnum = 78;                                         //样本的类数;



const int block_r = 3;                                             //图片block的半径大小；

const int r = 3;                                                   //采样半径为3；
...
const float sita_u = 1;  //激活函数的差，越小越好
const float sita_s = 1.5;  //自身分布的散度，越大越好
...
myLBVLD::myLBVLD() {
	...
	if (IsOverLapping == 1) {
		...
		block_stride = block_r * 2 + 1;   //等于2倍的block_r的长度其效果则为没有滑动窗口。
	}
	...
	//PDM矩阵的维度：D1*D2
	D1 = trainnum*classnum*Block_Size.area();// Sample_size*Sample_size - 1;       //w维度:D1*d1;

	D2 = (2 * r + 1)*(2 * r + 1) - 1;               //v维度:D2*d2;
}
...
void myLBVLD::Compute_DFDVec_KMeans() {
	//根据学习好的w,v特征生成DFD特征向量，并且将训练图片和测试图片生成的向量分开存放
...
std::vector<cv::Mat> myLBVLD::learn_w_and_v(std::vector<cv::Mat>PDM_img, cv::Mat  &w, cv::Mat &v) {
	...
	//计算每一类的均值和每一张图片的均值；
	cv::Mat sum_mean;   //所有训练集的均值；
	std::vector<cv::Mat> class_mean_vec;   //存放每一类的矩阵均值；
...
void myLBVLD::knn_classification()
{
	...
	fout << rate << std::endl;   //将识别率保存在res.txt文件下。
	fout.close();
}
...
void myLBVLD::LR_classification() {
	//效果不是很好，一般般，目前不太清楚是OpenCV代码问题或者是我调试问题。
	cv::Mat trainData, trainLabel, testData, testLabel;
